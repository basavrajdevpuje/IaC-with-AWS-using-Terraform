...
resource "aws_instance" "http_servers" {
  ...
  # creating EC2 instances in each of the subnet
  for_each  = data.aws_subnet_ids.default_subnet_ids.ids # as it is a set
...
resource "aws_elb" "classic_elb" {
  name            = "classicELB"
  subnets         = data.aws_subnet_ids.default_subnet_ids.ids # as we already have the ids in .tfstate, no need to use the for_each
